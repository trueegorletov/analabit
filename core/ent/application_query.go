...
func (aq *ApplicationQuery) QueryHeading() *HeadingQuery {
	query := (&HeadingClient{config: aq.config}).Query()
...
func (aq *ApplicationQuery) WithHeading(opts ...func(*HeadingQuery)) *ApplicationQuery {
	query := (&HeadingClient{config: aq.config}).Query()
...
//		GroupBy(application.FieldStudentID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Application.Query().
//		Select(application.FieldStudentID).
//		Scan(ctx, &v)
...
func (aq *ApplicationQuery) loadHeading(ctx context.Context, query *HeadingQuery, nodes []*Application, init func(*Application), assign func(*Application, *Heading)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Application)
	for i := range nodes {
		if nodes[i].heading_applications == nil {
			continue
		}
		fk := *nodes[i].heading_applications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "heading_applications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
