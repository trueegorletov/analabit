...
// sourceHint is for logging (URL or file path).
func parseAndLoadApplicationsInternal(f *excelize.File, competitionType core.Competition, headingCode string, applications chan<- source.ApplicationData, sourceHint string) error {
	sheetList := f.GetSheetList()
	if len(sheetList) == 0 {
	...
	for i := startRowIndex; i < len(rows); i++ {
		...
		// Send ApplicationData to the channel
...
func processApplicationsFromLists(
	applications chan<- source.ApplicationData, // Changed from vc *core.VarsityCalculator
	headingCode string,
	prettyName string,
	...
	for _, listDef := range definitions {
		...
		err = parseAndLoadApplicationsInternal(currentFile, listDef.CompetitionType, headingCode, applications, listDef.Source)
		if err != nil {
			log.Printf("Warning: Failed to load applications from %s (%s): %v. Continuing.",
				listDef.ListName, listDef.Source, err)
		}

		if closeCurrentFile && currentFile != nil {
