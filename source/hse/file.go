...
func (s *FileHeadingSource) LoadTo(headings chan<- source.HeadingData, applications chan<- source.ApplicationData) error {
	...
	return processApplicationsFromLists(applications, headingCode, prettyName, definitions, rcListFile, s.RCListPath, openLocalExcelFile)
}
